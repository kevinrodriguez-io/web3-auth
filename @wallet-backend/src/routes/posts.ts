...
import { rw, r, UserIdentifiablePost } from '../lib/drivers/database.js';
...
postsRouter.get(
  '/',
  web3Auth({ action: 'posts:get-all', allowSkipCheck: true }),
  async (_req, res) => {
    const pk = authorizedPk(res);
    const posts = await r((db) =>
      db.data!.posts.filter(({ userId }) => userId === pk),
    );
...
postsRouter.get(
  '/:postId',
  web3Auth({ action: 'posts:get-one', allowSkipCheck: true }),
  async (req, res) => {
    const pk = authorizedPk(res);
    const { postId } = req.params;
    const post: UserIdentifiablePost | null = await r(
      (db) => db.data!.posts.find((post) => post.id === postId) ?? null,
    );
    if (!post) {
